...
class BidController extends Controller
{
    ...
    public function postBid(Request $request , $ad_id){
        ...
        if ($ad->buy_now_price && $bid_amount >= $ad->buy_now_price) {
            ...
            if ($bid_amount > $currentMaxBid) {
                return back()->with('success', trans('app.your_bid_is_highest_message'));
            }
    ...
    public function postMaxBid(Request $request , $ad_id){
        ...
        if ($maxBidObj) {
            ...
            $currentMaxBidPlusIncreaser = $currentMaxBid + $ad->price_increaser;

            if ($currentUserThatHasHighestBid->id != $user->id) {

                // here you buy or you dont
                if ($ad->buy_now_price && ($currentMaxBidPlusIncreaser >= $ad->buy_now_price)) {
                    $event = $ad->events->first();
                    ...
                    $bid->ad_id = $ad->id;
                    $bid->user_id = $user->id;
                    $bid->bid_amount = $currentMaxBidPlusIncreaser;
                    $bid->is_accepted = 0;
                    $bid->save();
                }

                // you are not buying here, you have just overbidded our opponent
                $notification = new Notification;
                $notification->title = trans('app.overbidding_notification_title');
                $notification->text = trans('app.overbidding_message_notification', ['article_title' => $ad->title]);
                $notification->url = url('auction/' . $ad->id);
            ...
            if ($currentUserThatHasMaxBid->id != $user->id) {
                if ($currentUserThatHasMaxBid->email_notifications == 1) {
                    Mail::to($currentUserThatHasMaxBid->email)->send(new OverbiddingUsersBidMail($ad));
                }
            }

            return back()->with('success', trans('app.own_max_bid_raised'));
        } else {
            $bid = new Bid;
            $bid->ad_id = $ad->id;
            $bid->user_id = $user->id;
